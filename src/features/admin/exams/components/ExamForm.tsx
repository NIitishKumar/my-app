...
import type { CreateExamData } from '../types/exam.types';
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-48 overflow-y-auto border border-gray-300 rounded-lg p-3">
            {classes.map((cls) => (
              <label key={cls.id} className="flex items-center space-x-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={formik.values.classIds.includes(cls.id)}
                  onChange={(e) => {
                    const classIds = e.target.checked
                      ? [...formik.values.classIds, cls.id]
                      : formik.values.classIds.filter((id) => id !== cls.id);
